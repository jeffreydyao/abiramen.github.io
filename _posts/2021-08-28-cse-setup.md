...
Your computer has now generated two files: `id_rsa` and `id_rsa.pub`. The `id_rsa.pub` file is what's called a public key, while `id_rsa` is a private key. **Never share (or really, even open) your private key.** It's called a private key for a reason ~~(that is, because you should share it with the world)~~.
...
It's worth noting that several extensions may not work due to the nature of how the SSH FS extension works.
...
## Acknowledgements
* [spanishpear](https://github.com/spanishpear) for letting me know that CSE intends to encouage students to use this extension, leading me to unshelving this blogpost idea.
* [adaluong](https://github.com/adaluong) and [lukefisklennon](https://github.com/lukefisklennon) for reading over this post and providing feedback.

As always, this post does not necessarily represent the views of the School of CSE.
