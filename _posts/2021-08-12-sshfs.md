...
1. **Run the following command:** `sshfs -o idmap=user z5555555@loginX.cse.unsw.edu.au: ~/cse`, replacing with your own zID, and replacing the X with the last digit of your zID. For example, if your zID was z5123456, you'd use `login6.cse.unsw.edu.au`.
...
```bash
_SSHFS_ZID=z5555555
alias csemnt="sshfs -o idmap=user ${_SSHFS_ZID}@loginX.cse.unsw.edu.au: ~/cse"
...
```
lsof | grep "$HOME/cse" | sed "s/  */\\t/g" | cut -f 2 | while read PID; do kill $PID; done
```
